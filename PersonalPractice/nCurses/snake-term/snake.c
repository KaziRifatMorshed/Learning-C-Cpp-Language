...
int main(int argc, char *argv[]) {
  ...
  vec2d berry = {1 + rand() % (screen_width - 1),
  ...
  while (true) {
    ...
    // game over
    if (collide_snake(head) || head.x < 0 || head.x > screen_width ||
      ...
      while (true) {
        ...
        mvaddstr(screen_height - 1, screen_width - 15,
    ...
    for (int i = 0; i < num_berries; i++) {
      if (berries[i].x == head.x && berries[i].y == head.y) {
        ...
        while (collide_snake(berries[i]) ||
               (berries[i].x == head.x && berries[i].y == head.y)) {
    ...
    if (spawn_berry == true) {
      num_berries += 1;
      vec2d new_berry = {1 + rand() % (screen_width - 1),
                         1 + rand() % (screen_height - 2)};
      while (collide_snake(new_berry) ||
             (new_berry.x == head.x && new_berry.y == head.y)) {
