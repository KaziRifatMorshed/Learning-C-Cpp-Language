...
void renderentries() {
  lf_div_begin(((vec2s){lf_get_ptr_x(), lf_get_ptr_y()}),
               ((vec2s){(s.winw - lf_get_ptr_x()) - GLOBAL_MARGIN,
                        (s.winh - lf_get_ptr_y()) - GLOBAL_MARGIN}),
               true);
  ...
  for (uint32_t i = 0; i < s.todo_entries.count; i++) {
    ...
    if (s.crnt_filter == FILTER_COMPLETED && !entry->completed)
      continue;
    if (s.crnt_filter == FILTER_IN_PROGRESS && entry->completed)
      continue;
    if (s.crnt_filter == FILTER_LOW && entry->priority != PRIORITY_LOW)
      continue;
    if (s.crnt_filter == FILTER_MEDIUM && entry->priority != PRIORITY_MEDIUM)
      continue;
    if (s.crnt_filter == FILTER_HIGH && entry->priority != PRIORITY_HIGH)
      continue;

    {
      ...
      bool clicked_priority =
          lf_hovered((vec2s){lf_get_ptr_x(), lf_get_ptr_y()},
    ...
    }
    {
      ...
      lf_push_style_props(props);
      if (lf_image_button((
              (LfTexture){.id = s.removeicon.id, .width = 20, .height = 20})) ==
          LF_CLICKED) {
    ...
    }
    {
      ...
      lf_push_style_props(props);
      if (lf_checkbox("", &entry->completed, LF_NO_COLOR, SECONDARY_COLOR) ==
          LF_CLICKED) {
        serialize_todo_list(s.tododata_file, &s.todo_entries);
      }
      lf_pop_style_props();
...

    {
      ...
      if (lf_image_button(((LfTexture){
              .id = s.raiseicon.id, .width = texw, .height = texh})) ==
          LF_CLICKED) {
...
void rendernewtask() {
  ...
  // Description input field
  {
    ...
    props.border_width = 1.0f;
    props.border_color =
        s.new_task_input.selected ? LF_WHITE : (LfColor){170, 170, 170, 255};
  ...
  {
    bool form_complete =
        (strlen(s.new_task_input_buf) && selected_priority != -1);
    const char *text = "Add";
    const float width = 150.0f;
    ...
    lf_push_style_props(props);
    lf_set_line_should_overflow(false);
    lf_set_ptr_x_absolute(s.winw -
                          (width + lf_get_theme().button_props.padding * 2.0f) -
                          GLOBAL_MARGIN);
    lf_set_ptr_y_absolute(s.winh -
                          (lf_button_dimension(text).y +
                           lf_get_theme().button_props.padding * 2.0f) -
                          GLOBAL_MARGIN);
  ...
  lf_next_line();
  {
    ...
    LfTexture backbutton =
        (LfTexture){.id = s.backicon.id, .width = 20, .height = 40};
...
void entries_da_resize(entries_da *da, int32_t new_cap) {
  todo_entry **temp =
      (todo_entry **)realloc(da->entries, new_cap * sizeof(todo_entry));
  if (!temp) {
...
int main(int argc, char **argv) {
  ...
  if (argc > 1) {
    ...
    if (strcmp(subcmd, "--help") == 0 || strcmp(subcmd, "-h") == 0) {
      ...
      printf("\t-a, --add \"[desc]\" [priority]     Add a new task to the todo "
             "list\n");
      printf("\t-r, --remove [idx]                Remove a task with a given "
             "index from the list.\n");
      printf("\t-d, --done [idx]                  Mark a task with a given "
             "index as completed.\n");
      printf("\t-n, --not-done [idx]              Mark a task with a given "
             "index as not completed.\n");
      printf("\t-r, --raise [idx]                 Raises a task with a given "
             "index to the top.\n");
    } else if (strcmp(subcmd, "--list") == 0 || strcmp(subcmd, "-l") == 0) {
      printf("======== Your To Do ========\n");
      char *priorities_str[] = {"L", "M", "H"};
      sort_entries_by_priority(&s.todo_entries);
      for (uint32_t i = 0; i < s.todo_entries.count; i++) {
        todo_entry *entry = s.todo_entries.entries[i];
        printf("%i | (%s) [%c]: %s\n", i, priorities_str[entry->priority],
      ...
      if (strcmp(priority_str, "low") == 0)
        ...
        printf("todo: invalid priority given: '%s' (valid priorities: {low, "
               "medium, high})\n",
               priority_str);
        return EXIT_FAILURE;
      }
  ...
  while (!glfwWindowShouldClose(s.win)) {
    ...
    lf_div_begin(
        ((vec2s){GLOBAL_MARGIN, GLOBAL_MARGIN}),
        ((vec2s){s.winw - GLOBAL_MARGIN * 2.0f, s.winh - GLOBAL_MARGIN * 2.0f}),
        true);
